...
{
    NSArray* articles = board->articleCollection;
//    NSLog(@"%@",board->articleCollection);
    ...
    for (int i = 0; i < defaultNumOfCards; i++) {
        NSLog(@"%@",articles[i]);
        if([articles[i] objectForKey:@"article_img"] != nil) {
            OutlinkThumbnailVC *outlinkThumbnailVC = [[OutlinkThumbnailVC alloc] initWithNibName:@"OutlinkThumbnailVC" bundle:nil];
            [outlinkThumbnailVC initViewWithAttribute:articles[i]];
            [subViews addObject:outlinkThumbnailVC];
        } else {
            SimpleThumbnailVC *simpleThumbnailVC = [[SimpleThumbnailVC alloc] initWithNibName:@"SimpleThumbnailVC" bundle:nil];
            [simpleThumbnailVC initViewWithAttribute:articles[i]];
            [subViews addObject:simpleThumbnailVC];
